...
# Eva Dusek Jennings
# Aug 8, 2022 -- sqrt(traffic) used instead of traffic; random effects = agency/location
#----------------------------------------------
...
if(run_exploratory_code==TRUE) {
  ...
  #for TSS: AIC is best for location, and BIC is best for agency; go with location for now
...
if(run_exploratory_code==TRUE) {
  ...
  #For TSS, the model with random intercept is slightly better than without (LRT p-value = 0.004)
}
...
if (run_exploratory_code==FALSE) {
  load(file=here("..", "scripts", "formulas_and_aics", "TSS_m4.RData"))
}
...
Form4a <- formula(result ~ rain + sqrt_traffic + devAge2)#my.formulas.m4[[1]]  #AIC=1240.2
Form4b <- formula(result ~ rain + sqrt_CO2_road + devAge2) #my.formulas.m4[[5]]  #AIC=1245.7

#####  Best fit Model4  ####

Form4 <- Form4a  #the model with sqrt_traffic is preferable; better fit & more direct/ explainable relationship with TSS 
...
M4.sub1 <- update(Model4, . ~ . + rain:sqrt_traffic) #not significant (p=0.4174)
anova(Model4, M4.sub1)
M4.sub2 <- update(Model4, . ~ . + rain:devAge2)  #just barely significant (p=0.0458)
anova(Model4, M4.sub2)
M4.sub3 <- update(Model4, . ~ . + sqrt_traffic:devAge2)  #not significant (p=0.1423)
anova(Model4, M4.sub3)
#none of these are significant enough to include
...
save(TSS.coc2, TSS.r1X, TSS.vf1X, TSS.Form3, TSS.Form4, TSS.rain, file=here("..", "results", "Frequentist_TSS Models.RData"))
...
#Eva's summary:
# Model 3 has an AIC just 10 pts higher than Model 4; however, there are only 2 IND sites and 2 LDR sites; 
#   This model (#3) may not hold up when applied to other watersheds in Puget Sound area  
