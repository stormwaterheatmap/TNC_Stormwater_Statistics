...
pairs(coc %>% select(result, antecedant_dry_days_std, daymet_precip_std, daymet_3day_std,
      ...
      lower.panel=panel.smooth2, upper.panel=panel.cor, diag.panel=panel.hist)
#for copper, important predictors are likely: 
#  impervious (0.6), imperv_ground (0.5), trees (-0.5), noDev (-0.6), pm25 (0.6), traffic (0.6)
#  daymet14, 21 or 28day (all -0.2)

#important predictors for this COC
importantPreds <- c("impervious", "pm25", "traffic", "nodev", "trees", "paved", "daymet_28")

#likely most important predictors
pairs(coc[, c("result", "agency", "impervious_std", "logTree_cover_std", "logNoDev_std",
              "pm25_std", "logTraffic_std", "daymet_28daySR_std")], 
      lower.panel=panel.smooth2, upper.panel=panel.cor, diag.panel=panel.hist)
...
#   location, agency, landuse, month and year do show heteroskedasticity
...
#   For copper, no long-term trend in concentration over time, but there may be a monthly component?
...
coc2 <- coc %>%
  ...
  mutate(landuse = case_when(land_use == "LDR" ~ "1.LDR",
                             land_use == "HDR" ~ "2.HDR",
                             land_use == "COM" ~ "3.COM",
                             land_use == "IND" ~ "4.IND")) %>%
  mutate(landuse=as.factor(landuse)) %>%
  mutate(residential = case_when(landuse=="1.LDR" ~ "RES",
                                 landuse=="2.HDR" ~ "RES",
                                 landuse=="3.COM" ~ "NORES",
                                 landuse=="4.IND" ~ "NORES")) %>%
  mutate(residential=as.factor(residential))
...
anova(M.gls1X, M.gls2X, M.gls3X, M.gls4X, M.gls5X, M.gls6X, M.gls7X, M.gls8X, M.gls9X, M.gls10X, M.gls11X, M.gls12X, M.gls13X, M.gls14X, M.gls15X, M.gls16X)
...
E.16X <- resid(object=M.gls16X, type="normalized")  
plot.resids(M.gls16X, E.16X, "X")

#variance functions for best fit models so far
vf1X <- varComb(varIdent(form= ~1|agency), varConstPower(form= ~dry))
#vf2X <- varComb(varIdent(form= ~1|agency), varExp(form= ~rain7))
...
#For copper, the model with random intercept is significantly better than without!
...
#for copper, the random slope model has a singular convergence; stick with random intercept model
...
# #----Model with Variance Function 2----#
# M.vf2X <- gls(FormX, data=coc2, method="REML", weights=vf2X)
# 
# #Random intercept model; this is nested in the best fit model from Step 2, so can compare with a likelhiood ratio test
# M2.lme1X <- lme(data=coc2, FormX, random = ~1|agency, method="REML", weights=vf2X,
#                 control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# anova(M.vf2X, M2.lme1X)  #the likelihood ratio test p-value compares the two nested models, and whether they are significantly different
# #For copper, the model with random intercept is significantly better than without!
# 
# #Random slope models; with our dataset, these only makes sense for rain or dry.  Other predictors don't have enough data points along x-axis to generate a random slope reliably.
# M2.lme2X <- lme(data=coc2, FormX, random= ~1 + rain|agency, method="REML", weights=vf2X,
#                 control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# anova(M.vf2X, M2.lme1X, M2.lme2X)  #likelihood ratio test
# #for copper, the random slope model has a singular convergence; stick with random intercept model
# 
# #random structure for best fit model so far
# r2X <- formula(~1|agency)
# 
# #best random effects structure & residual plots to test it
# M2.rX <- lme(data=coc2, FormX, random = r2X, method="REML", weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# summary(M2.rX)
# E2.rX <- residuals(object=M2.rX, type="normalized")
# plot.resids(M2.rX, E2.rX, "X")
# 
# #Check for temporal correlation; look for patterns in Auto-correlation plot for residuals
# par(mfrow=c(2,2))
# plot(E2.rX~coc2$start_date, pch=16, col="cadet blue")
# acf(E2.rX, na.action=na.pass, main="Auto-correlation plot for residuals")  #look for lines extending past blue dashed range
# #for copper, no indication of temporal autocorrelation
# 
# #Check for spatial correlation
# mydata2 <- data.frame(E2.rX, coc2.longitude=jitter(coc2$longitude, amount=0.05), coc2.latitude=jitter(coc2$latitude, amount=0.05))  #jitter the X-Y coordinates, so that data points aren't on top of each other
# coordinates(mydata2)<-c("coc2.longitude","coc2.latitude")
# bubble(mydata2,"E2.rX",col=c("black","grey"), main="Residuals",xlab="X-coordinates", ylab="Y-coordinates")
# 
# #variogram to test for spatial correlation
# Vario1 = variogram(E2.rX ~ 1, mydata2)
# plot(Vario1)  #if there is no spatial correlation, will see a horizontal bar of points at the top of the plot
# Vario2 <- variogram(E2.rX ~ 1, mydata2, alpha = c(0, 45, 90,135) )
# plot(Vario2)  #do we see any different patterns in the different directions, or roughly the same pattern?
# #for copper, no indication of temporal autocorrelation
# 
...
# #----Model with Variance Function 1----#
# 
# #Find optimal fixed effect structure
# M1.fullX <- lme(data=coc2, FormX, random = r1X, method="ML", weights=vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# summary(M1.fullX)  #check to see if any effects are non-significant.  If so, remove them now
# M1.X <- M1.fullX
# M1.fullX2 <- lme(data=coc2, update(FormX, .~. +grass +trees), random = r1X, method="ML", weights=vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# summary(M1.fullX2)  #check to see if any effects are non-significant.  If so, remove them now
# 
# #models for formulas A, B and C, only landuse with no random structure, and landuse WITH random structure
# M1.A <- lme(data=coc2, FormA, random = r1X, method="ML", weights=vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# M1.B <- lme(data=coc2, FormB, random = r1X, method="ML", weights=vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# M1.C <- lme(data=coc2, FormC, random = r1X, method="ML", weights=vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# M1.LUr <- lme(data=coc2, result~landuse, random = r1X, method="ML", weights=vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# M1.LU <- gls(data=coc2, result~landuse, method="ML", control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# E1.A <- residuals(object=M1.A, type="normalized")
# E1.B <- residuals(object=M1.B, type="normalized")
# E1.C <- residuals(object=M1.C, type="normalized")
# E1.LUr <- residuals(object=M1.LUr, type="normalized")
# E1.LU <- residuals(object=M1.LU, type="normalized")
# plot.resids(M1.A, E1.A, "A")
# plot.resids(M1.B, E1.B, "B")
# plot.resids(M1.C, E1.C, "C")
# plot.resids(M1.LUr, E1.LUr, "A")  #land use only, but with random structure
# plot.resids(M1.LU, E1.LU, "A")  #land use only, NO random structure
# 
# AIC(M1.A, M1.B, M1.C, M1.ee1, M1.fullX, M1.LUr, M1.LU)
...
if(F) {
  ...
  hh <- ee[order(my.aics)]
  head(hh)
  #for copper, best AIC is for result~rain + paved + trees (any other predictors are irrelevant)
}

Form4 <- formula(result~rain + paved + trees)  #based on AIC's, best formula for model #4
...
if(F) {
  ...
  kk[1:10]
  #for copper, consistently best AICs are for result~landuse + rain + paved + traffic (can include other predictors)
}

Form5 <- formula(result~landuse + rain + paved + traffic)  #based on AIC's, best formula for model #5
...
#------ Model 2.5: land use with variance structure --------#

Model2.5 <- gls(data=coc2, result~landuse, method="ML", weights=vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
E2.5 <- residuals(object=Model2.5, type="normalized")
...
E3.rain <- residuals(Model3.rain, type="normalized")
boxplots.resids2(Model3.rain, E3.rain, FormX)


# Model3.5 <- lme(data=coc2, result~residential, random = r1X, method="ML", weights=vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# E3.5 <- residuals(object=Model3.5, type="normalized")
# 
# AIC(Model3, Model3.5)
...
M4.sub1 <- update(Model4, .~. +rain:paved) #slightly significant (p=0.0115)
anova(Model4, M4.sub1)
M4.sub2 <- update(Model4, .~. +rain:trees)  #not significant
anova(Model4, M4.sub2)
M4.sub3 <- update(M4.sub2, .~. +paved:trees)  #not significant
anova(M4.sub2, M4.sub3)  

Form4.int <- formula(result~rain + paved + trees + rain:paved)
...
# #------ Model 4.5: no land use; up to 3 predictors + rain; no random effect ---------#
# 
# Model4.5 <- gls(data=coc2, Form4, method="ML", weights=vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# E4.5 <- residuals(object=Model4.5, type="normalized")
...
boxplots.resids2(Model5, E5, FormX)

#try adding interactions and see if they are significant
M5.sub1 <- update(Model5, .~. +paved:rain)   #not significant
anova(Model5, M5.sub1)
M5.sub2 <- update(Model5, .~. +traffic:rain)  #not significant
anova(Model5, M5.sub2)
M5.sub3 <- update(Model5, .~. +landuse:rain)  #borderline, but not significant enough (p=0.041)
anova(Model5, M5.sub3)
M5.sub4 <- update(Model5, .~. +paved:traffic)  #significant (p=0.0058)
anova(Model5, M5.sub4)
M5.sub5 <- update(M5.sub4, .~. +landuse:paved)  #very significant (p<0.0001)
anova(M5.sub4, M5.sub5)
M5.sub6 <- update(M5.sub5, .~. +landuse:traffic)  #singularity - don't use!

Form5.int <- formula(result~landuse + rain + paved + traffic + paved:traffic + landuse:paved)
...
# M5.sub1 <- update(Model5, .~. +landuse:traffic)
# E5.sub1 <- residuals(object=M5.sub1, type="normalized")
# summary(M5.sub1)
# anova(Model5, M5.sub1)
# boxplots.resids2(M5.sub1, E5.sub1, FormX)

# #------ Model 5.5: super model: up to 3 predictors + landuse + rain; no random effect ---------#
# 
# Model5.5 <- gls(data=coc2, Form5, method="ML", weights=vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# E5.5 <- residuals(object=Model5.5, type="normalized")
# 
# Model5.2 <- update(Model5, .~. -landuse +residential)
# AIC(Model5, Model5.2)
# 
# Model5.3 <- update(Model5.2, .~. +residential:traffic)
# anova(Model5.2, Model5.3)
# summary(Model5.3)
# 
# AIC(Model5, Model5.2, Model5.3)
...
# M2.5.preds <- predict(Model2.5)
# plot.preds.vs.results(M2.5.preds)
...
# M4.5.preds <- predict(Model4.5)
# plot.preds.vs.results(M4.5.preds)
...
xyplot(bestE ~ traffic | agency, data=coc2, ylab="residuals", xlab="traffic",
       panel=function(x,y) {
         panel.grid(h=-1, v=2)
         panel.points(x, y, col=1)
         panel.loess(x, y, span=0.5, col=1, lwd=2) })
...
plot_model(Model4, sort.est=TRUE, show.values = TRUE, value.offset =.3, title="Copper: Parameter Estimates, Model 4")
plot_model(Model4.int, sort.est=TRUE, show.values = TRUE, value.offset =.3, title="Copper: Parameter Estimates, Model 4 w/int")
plot_model(Model5, sort.est=TRUE, show.values = TRUE, value.offset =.3, title="Copper: Parameter Estimates, Model 5")
plot_model(Model5.int, sort.est=TRUE, show.values = TRUE, value.offset =.3, title="Copper: Params, Model 5 w/int")

plot_model(M5.sub1, sort.est=TRUE, show.values = TRUE, value.offset =.3, title="Copper: Parameter Estimates for Landscape Predictors, Model 5")
...
#Formula 4: rain:paved
Plot.Quantile("rain", "paved", Model4.int, yEqn=6)
...
#Formula 5: paved:traffic, landuse:paved
Plot.Quantile("paved", "traffic", Model5.int, yEqn=6.7)

# Landuse:paved
IQ <- effect('landuse*paved', Model5.int,
             xlevels= list(paved= qList[["paved"]], landuse= levels(coc2$landuse)),
             se=TRUE, confidence.level=0.95, typical=mean)
IQ <- as.data.frame(IQ)

intEqnHDR <- "ln(Copper, HDR) = 2.51 + 2.64*paved + 1.38*HDR + 1.31*paved*HDR"
intEqnLDR <- "ln(Copper, COM) = 2.51 + 2.64*paved + 0.56*COM + -2.61*paved*COM"
intEqnIND <- "ln(Copper, IND) = 2.51 + 2.64*paved + 0.10*IND + -2.92*paved*IND"

ggplot() + 
  geom_line(data=IQ, size=1.5, aes(x=IQ$paved, y=fit, group=IQ$landuse, color=IQ$landuse))+
  ylab("COC")+ xlab("paved")+
  ...
  geom_point(data=coc2, aes(x=paved, y=result, fill=landuse), size=2.5, shape=21, stroke=0) + 
  scale_fill_manual(values=c("blue", "purple", "yellow", "orange"))+  #colors for points (fill)
  geom_text(aes(x=0.5, y=12, label=intEqnHDR), cex=4.5, color="black")+
  geom_text(aes(x=0.5, y=11.5, label=intEqnLDR), cex=4.5, color="black")+
  geom_text(aes(x=0.5, y=11, label=intEqnIND), cex=4.5, color="black")+
