# This script contains a function to run credibility intervals for an input watershed reduction,
#   and two scripts to plot credibility intervals (standardized & raw)
...
interpCI <- function(LCI=0.1, UCI=0.9, my.brm, my.coc2, wr_preds) {
  ...
  num_chunks <- ceiling(nrow(wr_preds)/3000)  #divide up wr_preds into chunks that are 3000 rows or less 
  for (i in 1:num_chunks) { #go through all groups of 3000 (or less) rows from wr_preds

    #select the proper chunk of wr_preds for this "i"
    if(i < num_chunks) {
      wr_preds_i <- wr_preds[((i-1)*3000+1) : (i*3000), ]   #corresponding rows of wr_preds for this "i"
    } else if (i ==num_chunks) {   #if it's the last "i", wr_preds_i should end at the last row of wr_preds
      wr_preds_i <- wr_preds[((i-1)*3000+1) : nrow(wr_preds), ]  
    }
...
plotBayesianCIs <- function(this.CI, this.chemical, preds) {
  ...
  mtext(paste(this.chemical, this.wr, "80% Credibility Intervals"),                   # Add main title
        side = 3,
        line = - 2,
