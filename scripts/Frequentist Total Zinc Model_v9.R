...
# Eva Dusek Jennings
# Aug 10, 2022 -- sqrt(traffic) used instead of traffic; nested random effects (1|agency/location)
#----------------------------------------------
...
#try adding interactions and see if they are significant
M4.sub2 <- update(Model4, . ~ . + rain:greenery)  #highly significant (p=0.0003)
anova(Model4, M4.sub2)
...
#try adding interactions and see if they are significant
M4.sub1 <- update(Model4, . ~ . + rain:sqrt_traffic) #slightly significant (p=0.0188)
anova(Model4, M4.sub1)
M4.sub2 <- update(Model4, . ~ . + rain:paved)  #highly significant (p=0.0006)
anova(Model4, M4.sub2)
...
#  models + interactions had similar AIC values, we concluded that Model 4c should be selected.

#####  Best fit Model4  ####
Form4c.int <- update(Form4c, .~. + rain:paved)
Model4c.int <- update(Model4c, .~. + rain:paved)
E4c.int <- residuals(object = Model4c.int, type = "normalized")
boxplots.resids2(Model4c.int, E4c.int, "X")


#------ Compare models and plot ---------#
AIC(Model1, Model3, Model4c, Model4c.int)

AIC(Model1, Model3, Model4c, Model4c.int)[,2] - AIC(Model4c.int)
...
M4.final <- lme(data=coc2, Form4c.int, random = r1X, method="REML", weights=vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
...
totZn.vf1X <- vf1X
totZn.Form3 <- Form3
totZn.Form4 <- Form4c.int
totZn.rain <- rain

save(totZn.coc2, totZn.r1X, totZn.vf1X, totZn.Form3, totZn.Form4, totZn.rain, file=here("..", "results", "Frequentist_Total Zinc Models.RData"))
...
totZn.M4 <- lme(data = coc2, Form4c.int, random = r1X, method = "REML", weights = vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
