...
pairs(coc %>% select(grass, paved, sqrt_CO2_road), 
      lower.panel=panel.smooth2, upper.panel=panel.cor, diag.panel=panel.hist)
...
best_predictors <- c("totRES", "intURB_IND", "grass", "greenery", "paved", "impervious", "trees", "sqrt_traffic",
                     "pm25_na", "sqrt_CO2_tot", "sqrt_CO2_com", "sqrt_CO2_nonroad", "roof_intURB_IND")
...
lp23 <- my.ggplot(23)
lp27 <- my.ggplot(27)
grid.arrange(lp3, lp5, lp6, lp7, lp8, lp10, lp12, lp13, lp16, lp20, lp21, lp23, lp27, nrow=4, ncol=4)
...
elements_to_remove <- c("greenery", "impervious", "roof_intURB_IND", "sqrt_CO2_nonroad")  #these predictors are highly correlated with others
...
if(run_exploratory_code==TRUE) {
  ...
  anova(M.gls1X, M.gls2X, M.gls7X, M.gls8X,
        M.gls18X, M.gls19X, M.gls20X, M.gls21X, M.gls22X)
  
  #for zinc: AIC is best for agency & dry (8, AIC=839), followed by just agency (2, AIC=847).  
  #  BIC is best for just agency (BIC=933), followed by agency & dry (8; BIC=934).
  ...
  boxplots.resids2(M.gls8X, E.8X, "X")
  
  E.2X <- resid(object=M.gls2X, type="normalized")
  
  #plot residuals for tech document
  par(mfrow=c(2,2), mar=c(4.5, 4.5, 4.5, 2))
  plot(fitted(M.gls1X), E.1X, ylab="normalized residuals", xlab="fitted values", main="no variance structure", col="gray", pch=16)
  plot(fitted(M.gls2X), E.2X, ylab="normalized residuals", xlab="fitted values", main="variance covariate: agency", col="gray", pch=16)
}

#variance functions for best fit model so far
vf1X <- varComb(varIdent(form= ~1|agency), varConstPower(form= ~dry))
vf2X <- varIdent(form=~1|agency)  #alternative, with only agency; in case "dry" ends up being eliminated from model...
...
if(run_exploratory_code==TRUE) {
  ...
  #for zinc, the random slope model is NOT better than random intercept (singular convergence)
}
...
if(run_exploratory_code==TRUE) {
  ...
  save(my.formulas.m4, my.aics.m4, file=(here("..", "scripts", "formulas_and_aics", "total_zinc_m4.RData")))
}

if (run_exploratory_code==FALSE) {
  load(file=here("scripts", "formulas_and_aics", "total_zinc_m4.RData"))
}
...
# aa <- grep("roof_nonRES", my.formulas.m4)  #ordered list (by AIC value) of all formulae that include roof_nonRES
...
qList <- list(paved=quantile(coc2$paved, probs=c(0,.25,.50,.75,1)),
               ...
               intURB_IND=quantile(coc2$pm25_na, probs=c(0,.25,.50,.75,1)),
...
Model3 <- lme(data=coc2, Form3, random = r1X, method="ML", weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
...
myForm <- my.formulas.m4[[8]]
#myForm <- update(my.formulas.m4[[3]], .~. -dry +pm25_na:rain)
...
myModel <- lme(data=coc2, myForm, method="ML", random=r1X, weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
...
#note: no "dry" predictor; variance covariate = agency
Form4a <- formula(result ~ rain + summer + sqrt_traffic + pm25_na + paved)  #my.formulas.m4[[3]]; AIC=795.6
Model4a <- lme(data=coc2, Form4a, method="ML", random=r1X, weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
E4a <- residuals(object=Model4a, type="normalized")
Model4a.int <- update(Model4a, .~. +rain:pm25_na)
summary(Model4a.int)

#note: no "dry" predictor; variance covariate = agency
Form4b <- formula(result ~ rain + summer + sqrt_traffic + pm25_na + greenery)  #my.formulas.m4[[4]]; AIC=801.2
Model4b <- lme(data=coc2, Form4b, method="ML", random=r1X, weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
E4b <- residuals(object = Model4b, type="normalized")
Model4b.int <- update(Model4b, .~. +rain:pm25_na)
summary(Model4b.int)

#note: no "dry" predictor; variance covariate = agency
Form4c <- formula(result ~ rain + summer + trees + pm25_na + intURB_IND)  #my.formulas.m4[[8]]; AIC=812.9
Model4c <- lme(data=coc2, Form4c, method="ML", random=r1X, weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
E4c <- residuals(object = Model4c, type="normalized")
Model4c.int <- update(Model4c, .~. +rain:pm25_na +rain:intURB_IND)
summary(Model4c.int)
...
M4.sub1 <- update(Model4, . ~ . + rain:sqrt_traffic) #not significant (p=0.0753)
anova(Model4, M4.sub1)
M4.sub2 <- update(Model4, . ~ . + rain:pm25_na)  #highly significant (p=0.0003)
anova(Model4, M4.sub2)


M4.sub3 <- update(Model4, . ~ . + rain:paved)  #significant (p=0.0023)
anova(Model4, M4.sub3)
M4.sub3b <- update(Model4, .~. + rain:pm25_na + rain:paved)  #rain:paved no longer significant! (p=0.102)
anova(M4.sub2, M4.sub3b)

M4.sub4 <- update(Model4, . ~ . + pm25_na:sqrt_traffic)  #barely significant (p=0.0319)
anova(Model4, M4.sub4)
M4.sub5 <- update(Model4, . ~ . + sqrt_traffic:paved)  #not significant (p=0.1761)
anova(Model4, M4.sub5)
M4.sub6 <- update(Model4, . ~ . + paved:pm25_na)  #barely significant (p=0.046)
anova(Model4, M4.sub6)

#Plot formula 4 interaction
Plot.Quantile("rain", "pm25_na", M4.sub2, yEqn=7.5)
myModel <- M4.sub2
boxplots.resids2(myModel, residuals(myModel, type="normalized"), "X")
#This relationship seems plausible, both on basis of data theoretically.  It also reduces AIC by 11-pts

#try using ALL seasons, not just summer vs not-summer.
M4.sub2c <- update(M4.sub2, .~. -summer + season)
summary(M4.sub2c)
boxplots.resids2(M4.sub2c, residuals(M4.sub2c, type="normalized"), "X")
#this does not lead to a better model; stick with just summer

Plot.Quantile("traffic", "pm25_na", M4.sub4, yEqn=7.5)  #don't include this -- it doesn't make sense from data standpoint
Plot.Quantile("traffic", "paved", M4.sub5, yEqn=7.5)  #don't include -- data spread is NOT compelling

Form4a.int <- update(Form4a, .~. +rain:pm25_na)
M4a.int <- update(Model4a, .~. +rain:pm25_na)
E4a.int <- residuals(object = M4a.int, type = "normalized")
boxplots.resids2(M4a.int, E4a.int, "X")


# #try this for Model 8 as well... (trees + pm25 + intURB_IND)
# Form4 <- Form4c
# Model4 <- Model4c
# E4 <- residuals(object = Model4, type="normalized")
# 
# 
# #try adding interactions and see if they are significant
# M4.sub1 <- update(Model4, . ~ . + rain:trees) #not significant (p=0.0765)
# anova(Model4, M4.sub1)
# M4.sub2 <- update(Model4, . ~ . + rain:pm25_na)  #highly significant (p=0.0001)
# anova(Model4, M4.sub2)
# 
# M4.sub3 <- update(Model4, . ~ . + rain:intURB_IND)  #highly significant (p<0.0001)
# anova(Model4, M4.sub3)
# M4.sub3b <- update(Model4, .~. + rain:pm25_na + rain:intURB_IND)  #still significant! (p=0.0032)
# anova(M4.sub2, M4.sub3b)
# 
# M4.sub4 <- update(Model4, . ~ . + pm25_na:trees)  #significant (p=0.0013)
# anova(M4.sub3b, M4.sub4)
# M4.sub5 <- update(Model4, . ~ . + trees:intURB_IND)  #significant (p=0.0056)
# anova(Model4, M4.sub5)
# M4.sub6 <- update(Model4, . ~ . + intURB_IND:pm25_na)  #just barely significant (p=0.046)
# anova(Model4, M4.sub6)
# 
# Plot.Quantile("rain", "pm25_na", M4.sub3b, yEqn=7.5)  
# Plot.Quantile("rain", "intURB_IND", M4.sub3, yEqn=7.5)  
# Plot.Quantile("trees", "pm25_na", M4.sub4, yEqn=7.5)  #don't include -- NOT compelling
# Plot.Quantile("trees", "intURB_IND", M4.sub5, yEqn=7.5)  #don't include -- NOT compelling
# Plot.Quantile("pm25_na", "intURB_IND", M4.sub6, yEqn=7.5)  #don't include -- NOT compelling
# 
# Form4c.int <- update(Form4c, .~. +rain:intURB_IND +rain:pm25_na)
# M4c.int <- update(Model4c, .~. +rain:intURB_IND +rain:pm25_na)
# E4c.int <- residuals(M4c.int, type="normalized")
# summary(M4c.int)
# boxplots.resids2(M4c.int, residuals(M4c.int, type="normalized"), "X")


# #####  Try the OLD Model 4, with roof_RES + devAge + traffic...  #####
# 
# #this model had really great fit with an old set of predictors for land use
# coc2$devAge2 <- coc$devAge2
# coc2$roof_totRES <- coc$roof_totRES
# Form4.old <- formula(result ~ rain + summer + traffic + devAge2 + roof_totRES)  #AIC=809.8
# Model4.old <- lme(data=coc2, Form4.old, method="ML", random=r1X, weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# E4.old <- residuals(object=Model4.old, type="normalized")
# boxplots.resids2(Model4.old, E4.old,)
# plot.single.preds(Model4.old)
# 
# theModel <- Model4.old
# if(class(theModel)=="gls")  aa <- summary(theModel)$coefficients
# if(class(theModel)=="lme")  aa <- summary(theModel)$coefficients$fixed
# aa <- c("traffic", "devAge2", "roof_totRES")
...
# #this model, which had great fit with an older set of predictors, does not look so good now... 
...
AIC(Model1, Model3, Model4a, M4a.int)

AIC(Model1, Model3, Model4a, M4a.int)[,2] - AIC(M4a.int)
...
# axis(side=1, at=c(2,3.8,5.2,7,10,13), labels=c("King", "Pie", "PoT", "Sea", "Sno", "Tac"))
# abline(h=0, col="gray")
plot(coc2$location, E4a.int, main="Landscape Predictor Model", ylab="Residuals", xaxt="n", col=colors_location)
axis(side=1, at=c(2,3.8,5.2,7,10,13), labels=c("King", "Pie", "PoT", "Sea", "Sno", "Tac"))
abline(h=0, col="gray")
...
M3.final <- lme(data=coc2, Form3, random = r1X, method="REML", weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
...
M4.final <- lme(data=coc2, Form4a.int, random = r1X, method="REML", weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
...
plot(Vario2)  #do we see any different patterns in the different directions, or roughly the same pattern?
#for Zinc, the bubble plot isn't perfect, but the variogram looks fine
...
save(totZn.coc2, totZn.r1X, totZn.vf1X, totZn.Form3, totZn.Form4, totZn.rain, file=here("..", "results", "Total Zinc Models.RData"))
...
totZn.M3 <- lme(data = coc2, Form3, random = r1X, method = "REML", weights = vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
totZn.M4 <- lme(data = coc2, Form4a.int, random = r1X, method = "REML", weights = vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
...
plot_model(M3.final, sort.est=TRUE, show.values = TRUE, value.offset =.3, title="Zinc: Parameter Estimates, Model 3")
...
#save important items with total zinc-specific names
totZn.coc2 <- coc2
totZn.r1X <- r1X
totZn.vf1X <- vf2X  #use the variance fxn with only agency as covariate
totZn.Form4 <- Form4a
totZn.Form4.int <- Form4.int

save(totZn.coc2, totZn.r1X, totZn.vf1X, totZn.Form4a, totZn.Form4e, file=here("results", "Total Zinc Models.RData"))
...
totZn.null <- gls(data = totZn.coc2, result ~ 1, method = "REML") 
totZn.M3 <- lme(data = totZn.coc2, result ~ landuse + rain, random = totZn.r1X, method = "REML", weights = totZn.vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
totZn.M4a <- lme(data = totZn.coc2, totZn.Form4a, random = totZn.r1X, method = "REML", weights = totZn.vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
totZn.M4b <- lme(data = totZn.coc2, totZn.Form4b, random = totZn.r1X, method = "REML", weights = totZn.vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
totZn.M5a <- lme(data = totZn.coc2, totZn.Form5a, random = totZn.r1X, method = "REML", weights = totZn.vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
totZn.M5b <- lme(data = totZn.coc2, totZn.Form5b, random = totZn.r1X, method = "REML", weights = totZn.vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
totZn.M5e <- lme(data = totZn.coc2, totZn.Form5e, random = totZn.r1X, method = "REML", weights = totZn.vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# totZn.M5 <- lme(data = totZn.coc2, totZn.Form5, random = totZn.r1X, method = "REML", weights = totZn.vf1X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))

totZn_models <- list(
  totZn.null = totZn.null,
  totZn.M3 = totZn.M3,
  totZn.M4a = totZn.M4a,
  totZn.M4b = totZn.M4b,
  totZn.M5a = totZn.M5a,
...
plotreg(totZn_models, custom.title = "Regression Results, Total Zinc", custom.model.names = names(totZn_models))
plot_models(totZn_models[-c(1)],m.labels = names(totZn_models[-c(1)]),legend.title = "Models", show.values = TRUE,show.intercept = TRUE)
