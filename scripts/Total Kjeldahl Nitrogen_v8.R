...
coc[which(coc$location_id=="TFWFD1" & coc$conc==255),]  #what is going on with this sample that has a strange
#  detection limit??  I looked it up in the raw data, and it is flagged with a "result data qualifier = U"...

#save a copy of the original concentrations, for censored statistics tests (see end of script)
coc$oconc <- coc$result  #original concentration
coc$ln.oconc <- log(coc$oconc)  #log-transformed original concentration

#Until we have a better methodology, substitute the ND's with half the detection limit
coc <- coc %>% 
...
# both gamma and log-normal distributions work.  Let's ln-transform concentration data in column "result"
...
lp25 <- my.ggplot(25)
lp27 <- my.ggplot(27)
grid.arrange(lp9, lp11, lp13, lp14, lp19, lp20, lp22, lp24, lp25, lp27, nrow=4, ncol=4)
...
elements_to_remove <- c("sqrt_CO2_road", "nodev", "sqrt_popn", "roof_intURB_IND")  #these predictors are highly correlated with others
...
if (run_exploratory_code==TRUE) {
  ...
  anova(M.gls1X, M.gls2X, M.gls4X, M.gls7X, M.gls8X,
        M.gls18X, M.gls19X, M.gls20X, M.gls21X, M.gls22X)
  #for TKN: AIC is best for agency & dry (8) followed by agency & rain (18)
  #  when landuse is removed from FormX, best AIC is for agency+dry, followed by just agency
  #  BIC is best for just agency
  
  #likelihood ratio tests for nested versions of the best fit model
  anova(M.gls1X, M.gls2X, M.gls8X)     #best for 14-day precip
  
  #residual plots for best fit models -- look for homogeneity of residuals
  E.8X <- resid(object=M.gls8X, type="normalized")      #best for 14-day precip
  boxplots.resids2(M.gls8X, E.8X, "X")

  E.2X <- resid(object=M.gls2X, type="normalized")      #best for 14-day precip
  boxplots.resids2(M.gls2X, E.2X, "X")
  
    
  par(mfrow=c(1,1), mar=c(4,4,4,1))
  plot(fitted(M.gls8X), E.8X, main="residuals", pch=16, col=colors_location[as.numeric(coc2$location)])
}

#variance functions for best fit models so far
vf1X <- varComb(varIdent(form= ~1|agency), varConstPower(form= ~dry))  #when FormX doesn't include Landuse, best AIC, but 2nd best BIC
vf2X <- varIdent(form= ~1|agency)  #when FormX doesn't include Landuse, 2nd best AIC, best BIC
vf3X <- varComb(varIdent(form= ~1|agency), varExp(form= ~rain))  #Tied for 2nd AIC when FormX doesn't include Landuse
#NOTE: after models have been constructed, test the variance function using just agency!
...
if (run_exploratory_code==TRUE) {
  ...
  #plots of residuals: no variance structure vs. variance structure; use model that only has agency as variance covariate!
  M.vf2X <- gls(FormX, data=coc2, method="REML", weights=vf2X)
  E.vf2X <- resid(object=M.vf2X, type="normalized")  
  ...
  plot(fitted(M.vf2X), E.vf2X, main="variance covariate: agency", xlab="fitted values", xaxt="s", ylab="normalized residuals", col="gray", pch=16)
...
if(run_exploratory_code==TRUE) {
  ...
  save(my.formulas.m4, my.aics.m4, file=(here("formulas_and_aics", "total_kjeldahl_nitrogen_m4.RData")))
}

if (run_exploratory_code==FALSE) {
  load(file=here("formulas_and_aics", "total_kjeldahl_nitrogen_m4.RData"))
}
...
myForm <- update(my.formulas.m4[[5]], .~. -dry)

#formulas that are worth considering (single plots of predictors make sense)
Form4a <- formula(result ~ rain + summer + traffic + devAge2) #my.formulas.m4[[3]]  
Form4b <- formula(result ~ rain + summer + traffic + nodev) #my.formulas.m4[[9]]  
Form4c <- formula(result ~ rain + summer + sqrt_CO2_road + devAge2) #my.formulas.m4[[8]]  

# myForm <- Form4c  
# we selected formula 4c because sqrt_CO2_road is a predictor we don't have to cap at a particular level,
#  as we do traffic.  It is also a bit more granular, which averages things out a bit.  All models had similar
#  sources coming out as important.
...
# Form4 <- Form4a
# Model4 <- lme(data=coc2, Form4, method="ML", random=r1X, weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
# E4 <- residuals(object = Model4, type = "normalized")
# 
# #try adding interactions and see if they are significant
# M4.sub1 <- update(Model4, . ~ . + rain:traffic) #not significant (p=0.45)
# anova(Model4, M4.sub1)
# M4.sub2 <- update(Model4, . ~ . + rain:devAge2)  #not significant (p=0.9286)
# anova(Model4, M4.sub2)
# M4.sub3 <- update(Model4, . ~ . + traffic:devAge2)  #barely significant (p=0.0193)
...
#try adding interactions and see if they are significant
M4.sub1 <- update(Model4, . ~ . + rain:sqrt_CO2_road) #not significant (p=0.8007)
anova(Model4, M4.sub1)
M4.sub2 <- update(Model4, . ~ . + rain:devAge2)  #not significant (p=0.9435)
anova(Model4, M4.sub2)
M4.sub3 <- update(Model4, . ~ . + sqrt_CO2_road:devAge2)  #significant (p=0.0043)
anova(Model4, M4.sub3)

#Plot formula 4 interaction
Plot.Quantile("sqrt_CO2_road", "devAge2", M4.sub3, yEqn=8.5)
#the data spread isn't convincing.  Leave this interaction out.
...
axis(side=1, at=c(2,3.8,5.2,7,10,13), labels=c("King", "Pie", "POT", "Sea", "Sno", "Tac"))
abline(h=0, col="gray")
# plot(coc2$location, E4, main="Model 4a: traffic + devAge2", ylab="Residuals", xaxt="n", col=colors_agency[c(1,1,1,2,3,4,4,4,5,5,5,6,6,6)])
# axis(side=1, at=c(2,3.8,5.2,7,10,13), labels=c("King", "Pie", "POT", "Sea", "Sno", "Tac"))
# abline(h=0, col="gray")
# plot(coc2$location, E4b, main="Model 4b: traffic + nodev", ylab="Residuals", xaxt="n", col=colors_agency[c(1,1,1,2,3,4,4,4,5,5,5,6,6,6)])
# axis(side=1, at=c(2,3.8,5.2,7,10,13), labels=c("King", "Pie", "POT", "Sea", "Sno", "Tac"))
# abline(h=0, col="gray")
plot(coc2$location, E4c, main="Landscape Predictor Model", ylab="Residuals", xaxt="n", col=colors_agency[c(1,1,1,2,3,4,4,4,5,5,5,6,6,6)])
axis(side=1, at=c(2,3.8,5.2,7,10,13), labels=c("King", "Pie", "POT", "Sea", "Sno", "Tac"))
abline(h=0, col="gray")
...
M3.final <- lme(data=coc2, result~landuse+rain+summer, random = r1X, method="REML", weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
...
M4.final <- lme(data=coc2, Form4c, random = r1X, method="REML", weights=vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
...
save(TKN.coc2, TKN.r1X, TKN.vf1X, TKN.Form3, TKN.Form4, rain, file=here("results", "Total Kjeldahl Nitrogen Models.RData"))
...
TKN.M3 <- lme(data = coc2, Form3, random = r1X, method = "REML", weights = vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
TKN.M4 <- lme(data = coc2, Form4, random = r1X, method = "REML", weights = vf2X, control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
...
#------------------------------------#
#  Censored Data Regressions vs GLS  #  is the method of dividing reporting limits by 2 acceptable for TKN?
#------------------------------------#

#For Model 4C, try a TKN gls model that can be compared to the cenreg model: (ND conc's at 0.5*detection limit (DL))
tkn.gls <- gls(as.formula(result ~ rain + summer + sqrt_CO2_road + devAge2), data=coc2, method="REML")
summary(tkn.gls)$coefficients
tkn.lm <- lm(as.formula(result ~ rain + summer + sqrt_CO2_road + devAge2), data=coc2)  #lm same as gls

#try a TKN gls formula that uses ln-transformed original concentrations, where ND values are at their detection limit
tkn.gls2 <- gls(as.formula(ln.oconc ~ daymet_14day_std + summer + sqrt_CO2_road + devAge2), data=coc, method="REML")
summary(tkn.gls2)$coefficients
summary(tkn.gls)$coefficients
### in this situation, it is better to use 0.5*DL than DL; values are closer to that of the cenmle model!

#try a Regression on Order Statistics (ros) model using NADA package and CO2_roads + devAge2
tkn.ros <- ros(obs=coc$result, censored=coc$nondetect_flag)  #, forwardT=NULL, reverseT=NULL)
tkn.df <- as.data.frame(tkn.ros)

# par(mfrow=c(2,1))
# hist(tkn.df$obs)
# hist(tkn.df$modeled)
# plot(tkn.df %>% filter(censored==TRUE) %>% select(obs, modeled) )
...
tkn.mle <- with(coc, cenmle(oconc, nondetect_flag, daymet_14day_std + summer + sqrt_CO2_road + devAge2))
tkn.reg <- with(coc, cenreg(Cen(oconc, nondetect_flag) ~ daymet_14day_std + summer + sqrt_CO2_road + devAge2, 
                            dist="lognormal"))
tkn.reg2 <- cenreg(Cen(coc$ln.oconc, coc$nondetect_flag) ~ coc$daymet_14day_std + summer + sqrt_CO2_road + coc$devAge2, 
                   dist="gaussian")
summary(tkn.reg)
summary(tkn.reg2)
summary(tkn.mle)$coefficients
summary(tkn.mle)$table[1:5, 1:2]  #this gives just the coefficients & their SE
#all three of these give the same result


summary(tkn.gls)$tTable[,1:2]

#is there a way to get estimated censored values following a distribution?
tkn.cen <- Cen(coc$ln.oconc, coc$nondetect_flag)  #an S4 object -- access slots using @ instead of $
tkn.cen1 <- as.vector(tkn.cen@Surv)
tkn.cen1 - coc$result
coc$oconc[which(coc$nondetect_flag==TRUE)]
exp(tkn.cen1[which(coc$nondetect_flag==TRUE)])
#not using Cen!  Cen just combines the results with the censored info, but doesn't estimate censored
#   values following a distribution...


#-------------------------------------------#
#  Try equation with no variance structure  #
#-------------------------------------------#

#the purpose of this is to understand the importance of the variance structure in predicting the correct
#  values of parameters, and whether the variance structure needs to be used in predictions

M4.full <- lme(data=coc2, Form4c, random = r1X, weights=vf2X, method="REML", control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
M4.noVS <- lme(data=coc2, Form4c, random = r1X, method="REML", control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
#E4.noVS <- residuals(object=M4.noVS, type="normalized")

M4.noRE <- gls(data=coc2, Form4c, weights = vf2X, method="REML", control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))

M4.noREnoVS <- gls(data=coc2, Form4c, method="REML", control = lmeControl(maxIter = 1e8, msMaxIter = 1e8))
...
visreg(Model4, "sqrt_CO2_road")
visreg(Model4, "summer")
visreg(Model4, "rain")
